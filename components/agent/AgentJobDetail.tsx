...
import { submitJobWork, createJobMediaUploadUrls, recordJobMedia, deleteJobMedia } from './agent-jobs.actions';
...
// Note (2026-09-26): kept in sync with agent-jobs.actions.ts's split
// upload API (see ARCHITECTURE.md #60) purely so this unwired legacy
// screen keeps compiling — AgentCapture.tsx/AgentCaptureScreen.tsx is the
...
export function AgentJobDetail({ job, media }: { job: any; media: any[] }) {
  ...
  function handleUpload(formData: FormData) {
    ...
    startUpload(async () => {
      const urlResult = await createJobMediaUploadUrls(job.id, files.map((f) => f.name));
      ...
      if (succeeded.length > 0) {
        ...
        const result = await recordJobMedia(job.id, items);
        if (result?.error) {
