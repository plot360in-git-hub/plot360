...
export function PlotDetailsForm({
  ...
  return (
    <form action={handleSubmit} className="card" style={{ maxWidth: 640, margin: '0 auto' }}>
      <h2 style={{ marginBottom: 24 }}>{mode === 'edit' ? 'Edit Property Details' : 'Plot Registration'}</h2>
      ...
      <h4 style={{ marginBottom: 12 }}>Plot Address</h4>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
        {CORNERS.map((c) => (
          <div key={c.key} className="card section-alt" style={{ padding: 16 }}>
